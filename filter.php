		...
		<div class="row">
			<form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" style="width:80%; margin:0 auto; padding:7px; background:#27da93" role="form">
			  ...
			  <div class="form-group col-md-2" style="padding-left:2px; padding-right:2px; margin-bottom:5px">
				   ...
				   <input type="text" name="locality" class="form-control" id="skills" size="50" placeholder="Locality" value="<?php if(isset($_POST['locality'])) { echo htmlentities ($_POST['locality']); }?>" />
			  </div>
			  <div class="form-group col-md-2" style="padding-left:2px; padding-right:2px; margin-bottom:5px">
		...
		<h4>Properties</h4>
			<div class="deal-top-top">
				...
				if(1==1)
				{
						...
						echo "<input type='text' name='a_locality' value='$locality' />";
						echo "<input type='text' name='a_any_hk' value='$any_hk' />";
						echo "<input type='text' name='furnish' value='$furnish' />";
						echo "<input type='text' name='rent' value='$rent' />";
						...
						$query = "SELECT * FROM property_details WHERE locality IN ($a_locality) && flat_type IN ('$a_any_hk') && furnishing IN ('$a_furnish') && monthly_rnet <= $rent ORDER BY id DESC LIMIT $start, $limit";
		...
		<div class="nav-page">
			<nav>
			  <?php
				...
				$rows=mysqli_num_rows(mysqli_query($connection,"SELECT * FROM property_details WHERE locality IN ($a_locality) && flat_type IN ('$a_any_hk') && furnishing IN ('$a_furnish') && monthly_rnet <= $rent"));
